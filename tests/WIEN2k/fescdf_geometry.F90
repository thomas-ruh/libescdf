module fescdf_geometry
    ...
    interface
        subroutine escdf_geometry_new(geometry) bind(c)
            import
            type(c_ptr) :: geometry
        end subroutine escdf_geometry_new
    end interface
    interface
        subroutine escdf_geometry_read_metadata(geometry, path) bind(c)
            import
    ...
    interface
        subroutine escdf_geometry_write_metadata(geometry, path) bind(c)
            import
            type(c_ptr), value :: geometry
            character(kind=c_char) :: path(*)
...
contains
    ...
    subroutine fescdf_geometry_new(geometry)
        type(fescdf_geometry_t), intent(inout) :: geometry

        call escdf_geometry_new(geometry%ptr)

    end subroutine fescdf_geometry_new

    subroutine fescdf_geometry_read_metadata(geometry, path)
        type(fescdf_geometry_t), intent(inout) :: geometry
        character(kind=c_char, len=*), intent(in) :: path

        call escdf_geometry_read_metadata(geometry%ptr, path)

    end subroutine fescdf_geometry_read_metadata

    subroutine fescdf_geometry_write_metadata(geometry, path)
        type(fescdf_geometry_t), intent(in) :: geometry
        character(kind=c_char, len=*), intent(in) :: path

        call escdf_geometry_write_metadata(geometry%ptr, path)
